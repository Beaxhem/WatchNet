...
//  
//
//  Created by Ilya Senchukov on 03.02.2022.
//
...
public class Reachability {
    ...
    public init(hostname: String) throws {
        guard let reachability = SCNetworkReachabilityCreateWithName(nil, hostname) else {
            throw Error.failedToCreateWith(hostname)
        }
    ...
    public init() throws {
        ...
        guard let reachability = withUnsafePointer(to: &zeroAddress, {
            ...
            throw Error.failedToInitializeWith(zeroAddress)
        }
        self.reachability = reachability
        isReachableOnWWAN = true
        try start()
...
private extension Reachability {
    ...
    func start() throws {
        ...
        guard SCNetworkReachabilitySetCallback(reachability, callout, &context) else {
			stop()
            throw Error.failedToSetCallout
        }
    ...
    func flagsChanged() {
        ...
        NotificationCenter.default.post(name: .networkChanged, object: self)
    }
